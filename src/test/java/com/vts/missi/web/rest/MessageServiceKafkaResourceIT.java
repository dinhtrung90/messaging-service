...
import com.vts.missi.config.KafkaProperties;
...
class MessageServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        MessageServiceKafkaResource kafkaResource = new MessageServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/message-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/message-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
